...
for (t in 1:Repeat) {
  ...
  # use package "penalizedLDA" #
  params = PenalizedLDA.cv(t(X_valid), as.numeric(Y_valid), lambdas = c(1e-4,1e-3,1e-2,.1,1,10), nfold = 5, type="ordered", lambda2 = .3)
  bestK = params$bestK
  lambda = params$bestlambda
  model = PenalizedLDA(t(X_train), as.numeric(Y_train), xte = NULL, type = "ordered", lambda = lambda, K = bestK, lambda2 = .3)
  Y_predict = as.factor(predict(model, t(X_test))$ypred)
